...
void Vector::calcAirportTraffic(int index) const
{
 ...
 for (i = 0; i < count; i++)
 	{
   passengers = (double) cityArray[index].getPop() * cityArray[i].getPop() / 2500000000U;
   distance = acos(
    sin(cityArray[index].getLat() * M_PI / 180) * sin(cityArray[i].getLat() * M_PI / 180) 
    + cos(cityArray[index].getLat() * M_PI / 180) * cos(cityArray[i].getLat() * M_PI / 180)
    * cos((cityArray[index].getLong() - cityArray[i].getLong()) * M_PI / 180)) * 3963;
  
  if (distance < 100)
    passengers = 0;
    
   totalPassengers += passengers;
   cout<<cityArray[i].getName()<<", "<<cityArray[i].getState()<<": "<<passengers<<"\n";
  }
