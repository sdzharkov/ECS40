...
City & City::operator=(const City& rhs) 
{
 ...
 //////////////////// this IS NEW
  ...
  if (rhs.state)
    {
    state = new char [strlen(rhs.state) + 1]; //////this is the errorcat 
  ...
  //strncpy(airport, rhs.airport);  //possibly this?
...
//void City::calcDistance(const City *city2) const
{
  ...
  passengers = (double) city1->population * city2->population / 2500000000U;
  distance = acos(
...
int City::showTraffic(const City *city1, const City *city2) const
//int City::showTraffic(const City *city2) const
{
int distance, passengers;
 
  passengers = (double) city1->population * city2->population / 2500000000U; // need to add this u
  distance = acos(
    sin(city1->latitude * M_PI / 180) * sin(city2->latitude * M_PI / 180) 
    + cos(city1->latitude * M_PI / 180) * cos(city2->latitude * M_PI / 180)
    * cos((city1->longitude - city2->longitude) * M_PI / 180)) * 3963;
