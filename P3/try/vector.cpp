...
void Vector::readAirports()
{
  ...
  City city;
  ifstream fp;
  fp.open("airportLL.txt");// apparently you don't need this
  //open("airportLL.txt");
...
{
  ifstream fp;
  fp.open("citypopulations.csv"); //don't need this
  //open("citypopulations.csv");
...
void Vector::calcAirportTraffic(int index) const
{
  ...
  for (i = 0; i < count; i++)
  {
    ...
    total += cityArray[index].showTraffic(&cityArray[index], &cityArray[i]);
    //total += cityArray[index].showTraffic(&cityArray[i]);
  } // if it compares add to total
